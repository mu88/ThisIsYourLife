...
public class SystemTests
{
    ...
    public async Task AppRunningInDocker_ShouldBeHealthy()
    {
        ...
        var healthCheckToolResult = await _container.ExecAsync(["dotnet", "/app/mu88.HealthCheck.dll", "http://127.0.0.1:8080/thisIsYourLife/healthz"], _cancellationToken);
    ...
    private static IContainer BuildAppContainer(INetwork network, string containerImageTag)
        => new ContainerBuilder($"webapp:{containerImageTag}-chiseled")
            .WithNetwork(network)
